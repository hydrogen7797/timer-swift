...
// MARK: - 現在のアプリ種別（3 ターゲット用: Info.plist の AppVariant で切り替え）

extension AppVariant {
    static var current: AppVariant {
        guard let raw = Bundle.main.infoDictionary?["AppVariant"] as? String,
              let variant = AppVariant(rawValue: raw) else {
            return .nowNotDecide
        }
        return variant
    }
